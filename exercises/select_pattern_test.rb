...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_words_with_three_letters
    ...
    selected = []
    words.each do |word|
      selected << word if ["bad", "cat", "dog", "red"].include?(word)
    end
  ...
  def test_pick_words_containing_e
    ...
    selected = []
    words.each do |word|
      selected << word if word.chars.include?("e")
    end
  ...
  def test_pick_dinosaurs
    ...
    animals.each do |animal|
      dinosaurs << animal if ["tyrannosaurus", "achillesaurus", "qingxiusaurus"].include?(animal)
    end
